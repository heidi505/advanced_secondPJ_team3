...
class FriendSearchViewModel extends StateNotifier<FriendSearchModel?> {
  ...
  void updateSearchKeyword(String newKeyword) {
    ...
    state = FriendSearchModel(keyword: newKeyword ?? "", friendSerchResponseDTO: state?.friendSerchResponseDTO ?? []);
  }
  ...
  Future<void> notifyInit() async {
    ...
    Logger().d("노티파이 진입");
    ResponseDTO responseDTO = await UserRepository().fetchFriendSearch(state!.keyword!, sessionUser.user!.jwt!);
...
// 3. 창고 관리자
final searchProvider = StateNotifierProvider.autoDispose<FriendSearchViewModel, FriendSearchModel?>((ref) {
  return FriendSearchViewModel(FriendSearchModel(friendSerchResponseDTO: []), ref);
});